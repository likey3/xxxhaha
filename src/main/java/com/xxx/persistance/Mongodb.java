...
//        map.put("time", new Date());
    ...
    /**
     * 利用一个网站的数据初始化标准的比赛队伍数据，数据库建好后 只能初始化一次
     * 否则会覆盖已经建好的数据库，以后新增的网站比赛对象也要归一化为建好的数据库中的对象
     *
     * @param entites 传入的比赛对象
     */
    public void InitOnce(List<GamblingEntity> entites) {
        entites.parallelStream().forEach(x -> {
            HashMap<String, Object> map = new HashMap<>();
            map.put("ID", x.getID());
            map.put("names", x.getNames());
            Document document = new Document(map);
            gambleDoc.insertOne(document);
        });
    }
